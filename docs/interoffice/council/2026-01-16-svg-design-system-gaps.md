...
- The `rx="8"` applies to the *container* rect, not the individual tab shapes
...
1. **"Key Concepts" vs "Additional Requirements"**:
   - "Key Concepts" is semantically correct. It answers: *"What mental model does the user need to understand this screen?"*
...
3. **Content Guidelines** (expanding on UIDesigner's examples):
   ...
   **Framing question for generators**: *"What does the user need to understand to successfully use this screen?"*
...
*Note: UIDesigner contributed valuable visual specs but is a contributor role, not a council voting member.*
