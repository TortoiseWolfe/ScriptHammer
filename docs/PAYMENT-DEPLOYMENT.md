...
| **Outbound Edge Functions** (browser → provider checkout creation)                        | ✅ all 8 deployed + live-proven: Stripe one-off (PR #155), PayPal one-off (PR #160), Stripe subscription + cancel/resume (PR #167). Only the PayPal-subscription **client wiring** remains ([#104](https://github.com/TortoiseWolfe/ScriptHammer/issues/104)) |
...
**Operator note:** the Stripe tab (one-off + Subscribe) and the PayPal tab (one-off) on `/payment-demo`, plus subscription **cancel/resume** on `/payment?tab=subscriptions`, all work once sandbox keys are configured — each leg has been driven against the real sandbox. PayPal **subscriptions** are the one unwired leg ([#104](https://github.com/TortoiseWolfe/ScriptHammer/issues/104)): the recurring PaymentButton shows a clear error on the PayPal tab.
