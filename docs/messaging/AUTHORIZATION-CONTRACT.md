...
| **C3**      | An **accepted `user_connections` row is required to create a 1:1 conversation** — in EITHER direction (`unique_connection` is `(requester_id, addressee_id)` and is not symmetric). Finding an EXISTING conversation is participant-scoped and connection-independent (C1).                                                                                                                                                                                                                                                          | RLS `conversations` INSERT `:1847`; .NET `MessagingQueries.HasAcceptedConnection` + `ConversationsController.Create`                                                                                                                                                                                                                                                                                                                    | `getOrCreateConversation` / `POST /api/messaging/conversations`        | ✅           |
