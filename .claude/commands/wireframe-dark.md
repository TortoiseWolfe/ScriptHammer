---
description: Generate SVG wireframes (dark theme) from a spec and add them to the interactive viewer. **BACKEND/ARCHITECTURE ONLY - NO MOBILE FRAMES**.
---
...
**⚠️ IMPORTANT: Dark theme is ONLY for backend/architecture diagrams.**
- NO mobile phone frames in dark theme
- If the wireframe needs a mobile view → use `/wireframe` (light theme) instead
- Mobile frames = front-end UX = light theme ALWAYS
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1400 800" width="1400" height="800">
  ...
  <defs>
    ...
    <style>
      /* Typography - AAA compliant, use these classes, not inline sizes */
  ...
  <text x="700" y="28" text-anchor="middle" font-family="system-ui, sans-serif" font-size="14" font-weight="600" fill="#8494a8" letter-spacing="1">WIREFRAME TITLE</text>

  <!-- ARCHITECTURE CONTENT AREA -->
  <!-- Dark theme is for backend/architecture diagrams ONLY - NO MOBILE FRAMES -->
  <!-- Full canvas: x=40, y=60, width=1320, height=680 -->
  <g id="architecture-view">
    <!-- Use boxes, arrows, and labels to show system components -->
    <!-- Panels: fill="#1e293b" or fill="#334155" -->
    <!-- Borders: stroke="#475569" -->
  </g>
</svg>
```

**Layout Dimensions** (Architecture Diagrams):
| Section | Position | Size | Notes |
|---------|----------|------|-------|
| Full canvas | x=40, y=60 | 1320×680 | Architecture diagrams |
| Margin | 40px | all sides | Padding from edge |

**⚠️ NO MOBILE FRAMES** - Dark theme is for backend/architecture only.
If your wireframe needs a mobile view, use `/wireframe` (light theme) instead.

**Component Sizing Standards** (architecture elements):
| Component | Size | rx |
|-----------|------|-----|
| System box | 200×100 | 8 |
| Database icon | 60×60 | 6 |
| Arrow | stroke-width=2 | - |
| Badge (pill) | height=22 | 11 |
| Card/Panel | varies | 8 |
