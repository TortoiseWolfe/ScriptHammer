...
describe('RLS: Audit Trail Protection (US5)', () => {
  let userA: TestUser;
  let userB: TestUser;
  let testAuditLogId: string;

  beforeAll(async () => {
    userA = await createTestUser(TEST_USERS.userA.email, TEST_USERS.userA.password);
    userB = await createTestUser(TEST_USERS.userB.email, TEST_USERS.userB.password);
  ...
  it('audit logs have valid event types', async () => {
    ...
    const { data, error } = await serviceClient.from('audit_logs').select('event_type');
