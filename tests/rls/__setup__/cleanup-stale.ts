...
import { assertRlsTargetApproved } from '../../fixtures/rls-target';
...
export async function setup() {
  ...
  // Before anything holding the service-role key exists (#1234). This hook is the first code
  // the suite runs and it DELETES users, so it is where a mis-pointed run must stop. Name the
  // target either way — a destination you can read is one you can question.
  assertRlsTargetApproved();
  console.log(`[rls cleanup-stale] target ${SUPABASE_URL}`);
