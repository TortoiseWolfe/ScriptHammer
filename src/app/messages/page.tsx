...
function MessagesContent() {
  ...
  useEffect(() => {
    const checkKeys = () => {
      // Simply check if keys are in memory - don't check database migration status
      const keys = keyManagementService.getCurrentKeys();
      if (!keys) {
        // Keys not in memory - need to re-authenticate to derive them
        setNeedsReAuth(true);
      }
  ...
  const handleReAuthSuccess = useCallback(() => {
    ...
    // Messages will load via useEffect when needsReAuth becomes false
  }, []);

  // Show loading while checking keys
  if (checkingKeys) {
    return (
